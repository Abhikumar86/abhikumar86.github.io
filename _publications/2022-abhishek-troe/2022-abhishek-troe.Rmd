...
data.frame(
    ...
    gt(caption = "List of keywords used for developing search string") |>
    tab_theme()
```
...
data.frame(
    ...
    group_by(Type) |> 
    gt(caption = "Eligibility criteria for inclusion or exclusion of studies") |>
    tab_theme()
```
...
**Abstract screening**: Our abstract screening resulted in exclusion of 264 articles and we are left with only 111 articles. Then we filtered these 111 articles and saved as the included articles (Figure \@ref(fig:fig1)). 
...
**Full-text screening**: For full text screening, we first selected five variables (`label`, `title`, `journal`, `year`  and `doi`) from the included abstracts. Then, we manually  screened articles based on the full-text of every article. This process excluded 71 more articles and we left with only 40 studies 
(Figure \@ref(fig:fig1)).

### Data extraction

We developed a search string based on the DOI (digital object identifier) and title (if DOI is not available) of included studies to extract bibliometric metadata from the Scopus database. We retrieved bibliometric information for 39 articles on 09 May 2021. However, we could not extract metadata for an article because it was not available in Scopus; therefore, we excluded it from the final systematic map and narrative synthesis (Figure \@ref(fig:fig1)). This bibliometric data had several missing values (marked as `NA`); these values were manually added from the full text, if available. 
...
* the domain of plant ecology for each study. Adopting a single unique classification system for ecological studies is difficult as study areas often overlap. However, we adhered to a previously proposed comprehensive classification system for the present study (Table \@ref(tab:tab3)).
...
data.frame(
    ...
    gt(caption = "Classification scheme for included studies") |> 
    tab_theme()
```

Next, we manually assigned an ecological domain according the above classification system (Table \@ref(tab:tab3)).
...
Our systematic literature search yielded a total of 2,888 documents from both Scopus and Web of Science Core Collection. After de-duplication and article screening, these records were reduced to 40 studies (Figure \@ref(fig:fig1)). However, only 39 studies were included in the final synthesis as bibliographic metadata was not available for a study (Figure \@ref(fig:fig1)). Thus, this synthesis is based on 39 studies published in 26 sources (journals) from 1988 to 2021. The results of descriptive analysis of bibliometric indicators for selected studies are presented in Table \@ref(tab:tab4).
...
read.csv("./data/main_information.csv") |>
    ...
    group_by(category) |>
    gt(caption = "General description of extracted bibliometric indicators from Scopus") |>
    tab_theme()
```

The studies identified for the present review were published in 26 different journals. Further, only nine journals had represented more than one study identified in this article (Figure \@ref(fig:fig2)). These nine journals (about 34.6%) represented about 56.4% of the total studies (n = 22) included. The maximum number of studies (n = 4) were published in 'International Journal of Remote Sensing' (Figure \@ref(fig:fig2)).

```{r fig2, fig.height=6, fig.width=8, fig.cap="Publication sources (journals) representing at least two studies identified in the present article. The number of publications in a particular journal is represented in parenthesis"}
...
read.csv("data/extracted_data.csv", fileEncoding = "UTF-8-BOM") |>
    ...
    filter(Freq > 1) |> 
    mutate(journal = paste0(Var1, " (", Freq, ")")) |>
...
Citations of an article usually related with its impact and influence on a specific research field. Highly cited articles are considered to have substantially contributed to the advancement of the field. The articles identified in this review exhibited about 20 citations per article at the average rate of about two citations per article per year (Table \@ref(tab:tab4)). The present analysis found that the article by @Yang2013 had the highest citation (n = 236) in which they used MaxEnt modelling to predict the distribution of *Justicia adhatoda* (Figure \@ref(fig:fig3)). Other highly cited articles with their respective citations are represented in Figure \@ref(fig:fig3).

```{r fig3, fig.height=8, fig.width=10, fig.cap="Most cited articles (with at least 25 citations) from idenitfied peer-reviewed publications"}

results$MostCitedPapers[1:9,] |> 
...
A total of 368 'indexed keywords' and 160 'author's keywords' were identified from  selected studies (Table \@ref(tab:tab4)). The connections among keywords were analysed using a keyword co-occurrence network of 'indexed keyword' which revealed three distinct patterns about the research field (Figure \@ref(fig:fig4)). The keyword 'India' and 'himalayas' were most dominant as suggested by largest nodes (Figure \@ref(fig:fig4)).

```{r fig4, fig.height=8, fig.width=8, fig.cap="Co-occurrence network of indexed keywords (n = 25) from Scopus. The size of each vertex is proportional to its degree (i.e. the frequency of keywords)"}
...
Our study identified a total of 111 distinct authors (Table \@ref(tab:tab4)). An analysis of the authors' collaboration network suggested that authors form  five distinct research groups working in different domains of plant ecology (Figure \@ref(fig:fig5)). The largest node for author 'padalia h' suggests that this researcher is highly productive and has published highest number of documents. Further, high weight of edges among the authors 'padalia h', 'nandy s', 'chauhan p' and 'srinet r' manifests that these authors frequently appear together i.e., they work as a team (Figure \@ref(fig:fig5)).

```{r fig5, fig.height=8, fig.width=8, fig.cap="Collaboration network of authors (n = 25) from the selected studies. The size of each vertex is proportional to its degree (i.e. the frequency of authors appearance)"}
...
The distribution of estimated study sites is depicted in Figure \@ref(fig:fig6). The spatial distribution of studies suggest a high density around Uttarakhand state followed by Haryana (Figure \@ref(fig:fig6)). Our selected studies represented three states (Himachal Pradesh, Haryana, and Uttarakhand) and two union territories (Jammu and Kashmir and Chandigarh) of India (excluding studies conducted in multiple regions or whole region) and only five studies covered the whole region. Among the geographical regions, about 59% of total studies were featured from the Uttarakhand (n = 23) only (Figure \@ref(fig:fig6)). On the other hand, not a single study was identified from the Siwalik region of Punjab and Uttar Pradesh.
...
vp <- grid::viewport(x = 0.925, y = 0.98, width = grid::unit(2.5, "inches"), 
		   ...
		   just = c("right", "top"))
## save the map
tmap_save(main_map, filename = "./data/map.png", insets_tm = inset_map, 
          insets_vp = vp, height = aspmain*7, width = 7, units = "in", dpi = 600)
rm(list = ls())
```

```{r fig6, out.width="100%", fig.cap="Geographic distribution of the study sites (reported or estimated from publications) across the study region (JK = Jammu and Kashmir, HP = Himachal Pradesh, CH = Chandigarh, HR = Haryana, UK = Uttarakhand). The number of studies per region is indicated in parenthesis"}
...
The number of publications has shown an overall increasing trend and exhibited  an annual growth rate of about 4.3% (Figure \@ref(fig:fig7)). During 2010s  annual production of scientific publications observed a steep rise and the year  2020 recorded highest number of publications (n = 6) within a single year  (Figure \@ref(fig:fig7)). Further, the decadal frequency of publications was six during 1990s, which dipped to only two publication per decade during 2000s. However, it so steeply increased during 2010s that it became double of all studies published from 1980s to 2000s (Figure \@ref(fig:fig7)). 

```{r fig7, fig.height=6, fig.width=8, fig.cap="Annual frequency of scientific publications included in this synthesis. The trend line is fitted using 80% LOESS (y ~ x) smoothening. Inset shows the decadal frequency of publications"}
...
We found three distinct study types, six distinct study region and seven distinct domains of plant ecology (Figure \@ref(fig:fig8)). Among the selected 39 studies, experimental studies were under-represented, accounting for just about 5% of total studies. On the other hand, observational and modelling studies were heavily dominated constituting about 95% of all the studies (Figure \@ref(fig:fig8)).

```{r fig8, fig.cap="Breakdown of selected studies by study type (EX = experimental, MO = modelling, OB = observational), region (CH = Chandigarh, HR = Haryana, HP = Himachal Pradesh, JK = Jammu and Kashmir, UK = Uttarakhand, WR = whole region) and domains of ecology (AE = applied ecology, CE = community ecology, EE = ecosystem ecology, LE = landscape ecology, PL = palaeoecology, PE = physiological ecology, SI = species interactions). The number of publications within each category is indicated in parenthesis", out.width="100%"}
...
df |> 
    ...
    mutate(
        x = factor(x, labels = c("study" = "Study type", 
                                 "location" = "Region", 
                                 "domain" = "Domain"),
                   levels = c("study", "location", "domain"))
    ) |>
...
We identified seven distinct (not mutually exclusive) domains under plant ecology (Figure \@ref(fig:fig8)). Among these assigned domains, about 38.5% studies belonged to landscape ecology and 30.77% studies belonged to ecosystem ecology; thus, together constituting about 70% of total studies (Figure \@ref(fig:fig8)).
