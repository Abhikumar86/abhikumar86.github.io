...
```{r fig1-map, echo=FALSE, fig.cap="The geographic distribution of the Siwaliks in the North-western region of India. Data were acquired from the Bhukosh portal of the Geological Survey of India (http://bhukosh.gsi.gov.in/)"}
...
```{r fig2, fig.cap="The distribution of Spelling variants (in the title of publications) among different scientific databases"}
library(ggplot2)
...
```{r fig3, fig.cap="The trend of annual scientific production of publications with spelling variants from both the Scopus and Web of Science database. The smooth lines were fitted using the loess (y ~ x) method"}
library(ggplot2)
...
```{r fig4, fig.height=8, fig.width=6, fig.cap="The distribution of terms in the countries of the corresponding author"}
library(ggplot2)
...
```{r fig5, fig.height=11, fig.width=8, out.width='90%', fig.cap="The distribution of terms in the top 10 most relevant sources of publications (Journals, Books, etc.) for each term in Scopus and Web of Science database"}
library(ggplot2)
...
read.csv("data/biblio-table.csv") |> 
    gt(caption = "Bibliometric indicators for the publications with spelling variants") |>
