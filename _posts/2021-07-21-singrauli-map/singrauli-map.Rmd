...
```{r eval=FALSE}
# extent for target site as left, bottom, right, upper
bb <- c(82.48, 24.1, 82.8, 24.2) 

# download data from SRTM
dem <- getData(name = "SRTM", lon = 84, lat = 24, path = "./dem/") 

# cropped to target site
dem_crop <- dem %>% crop(extent(82.48, 82.8, 24.1, 24.2)) 

# remove dem from environment
rm(dem) 

# extract slope from dem_crop
slope <- terrain(dem_crop, opt= "slope") 

# extract aspect from dem_crop
aspect <- terrain(dem_crop, opt= "aspect")

# prepare hill shade
# 45 altitude angle and 315 azimuth angle
dem_hs <- hillShade(slope = slope, aspect = aspect, angle = 45, direction = 315) 

# write to local directory
writeRaster(x = dem_hs, filename = "./data/dem_hs.tif", overwrite = T) 

# removed slope and aspect files
rm(slope, aspect) 
```
